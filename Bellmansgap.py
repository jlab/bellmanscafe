...
res=""
dirstr=""
#glob.glob('*.gap') returns a list of names of all files in the directory that end on ".gap"
...
def bellman():
    ...
    #Values of the combo-boxes that the user had selected when submitting is saved in this variable
    ...
    #This if statement only occurs after the user pressed the submit button
    if request.method == 'POST':
        #all variables are made global, this is a workaround, since initializing the variables as global did not work for some reason
        ...
        #the dictionary user_form_input is later returned to the html page in order
        ...
        #the previous selection (before pressing submit) in the results part of the page
        inputreminderlist = []
    ...
    if len(exlist) != 0 and program != "" and gra != "" and alg1 != "" and alg2 == "":
        ...
        #variables that are important for building the result part of the page and the previous selection of the combo-boxes
        #is returned back to the html page here
        #some of these variables might be outdated and unnecessary to return by now, further cleanup will follow
        return render_template("bellman.html", result=res, program=program, gra=gra, gapfiles=json.dumps(gapfiles), gramdict=json.dumps(gramdict), algdict=json.dumps(algdict), infotextsdict=json.dumps(infotextsdict), inputstringsnumberdict = inputstringsnumberdict, inputreminderlist=inputreminderlist, exlist=exlist, user_form_input=json.dumps(user_form_input))
    ...
    #if this return statement is reached then at least one combo-box necessary for execution was not selected/left on the default value
    #some of these variables might be outdated and unnecessary to return by now, further cleanup will follow
    return render_template("bellman.html", program=program, gra=gra, gapfiles=json.dumps(gapfiles), gramdict=json.dumps(gramdict), algdict=json.dumps(algdict), infotextsdict=json.dumps(infotextsdict), inputstringsnumberdict = inputstringsnumberdict, inputreminderlist=inputreminderlist, exlist=exlist, user_form_input=json.dumps(user_form_input))


#this function executes three commands in the shell and returns the result of the last command
#the first command is the gapc tool itself, 
#the second is the make command 
#and the third is the executable that was generated by the previous command
def calculategapc(program, command, name, exlist):
    #dirstr is the string of the directory where generated files regarding this program are safed
    ...
    #gapc Command
    #if this combination of algebras and operator has not been computed for this program yet it will be computed
    if not os.path.exists (dirstr+"/"+name+"_gapc.cc"):
        #if the directory that holds the created files does not exist yet for this program, it will be created
        ...
        #the returncode will be used to check if the process has run successfully
        pro1_returncode = pro1.returncode
        #the result of the process is saved in list1 and appended to the res list
        list1 = pro1.stdout.splitlines()
        list1.insert(0, "<b>Command</b>: " + commandstring)
        res.append(list1)
    #if this combination of algebras and operator has already been computed, it will not be computed again
    else:
    	list1 = []
    	list1.append("<b>Command</b>: " + commandstring)
    	res.append(list1)
    
    #further commands will be executed from within the directory to which the files have been saved
    os.chdir("./"+dirstr)
    
    #the commandstring is now rebuild as a make command
    commandstring = "make -f "+name+"_gapc.mf"+" 2>&1"
    #if the returncode was not 0, an error must have occured, this is however not handled yet, 
    #rather the rest of the code is just not executed in that case
    if pro1_returncode != 0:
    	errorstring = "There was an error"
    else:
        #if no error occured in process 1, process 2 is started via subprocess.run
        pro2 = subprocess.run(commandstring, shell=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        #the results of the make command are saved in list2, which is appended to the res list
        list2 = pro2.stdout.splitlines()
        list2.insert(0, "<b>Command</b>: " + commandstring)
        res.append(list2)
	    
	    #since the third command takes in a string input directly typed in by the user
        ...
        #the commandstring will also be built, but will only be displayed in the result section of the page
        ...
        if pro2.returncode != 0:
            ...
            #using a commandlist instead of a commandstring is supposed to act as a barrier for hacking attempts
            #with a commandlist the first element is interpreted as the program or command, 
            #while the rest of the elements are interpreted as arguments or parameters to that program or command,
            #therefore shell commands in the inputstring are not interpreted as commands but rather as arguments
