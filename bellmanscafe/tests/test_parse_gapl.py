...
from bellmanscafe.parse_gapl import _extract_comments, _parse_gapl_header, _merge, _parse_gapl_signature, parse_gapl
...
class ParseGAPLTests(TestCase):
    ...
    def test_merge(self):
        ...
        obs = _merge({'algebras': ['alg_mfe']},
                     {'grammars': ['fold']})
        self.assertEqual({'algebras': ['alg_mfe'], 'grammars': ['fold'], 'comments': {}}, obs)

        obs = _merge({'algebras': ['alg_mfe'], 'comments': {'a': 'Hallo'}},
                     {'grammars': ['fold']})
        self.assertEqual({'algebras': ['alg_mfe'], 'comments': {'a': 'Hallo'}, 'grammars': ['fold']}, obs)

        obs = _merge({'algebras': ['alg_mfe'], 'comments': {'a': 'Hallo'}}, {'grammars': ['fold'], 'comments': {'b': 'Welt'}})
        self.assertEqual({'algebras': ['alg_mfe'], 'comments': {'a': 'Hallo', 'b': 'Welt'}, 'grammars': ['fold']}, obs)

        with self.assertRaisesRegex(ValueError, 'Conflicting'):
            _merge({'algebras': ['alg_mfe'], 'comments': {'a': 'Hallo'}}, {'grammars': ['fold'], 'comments': {'a': 'Welt'}})

        obs = _merge({}, {'imports': [], 'inputs': ['rna'], 'comments': {'header': 'kurt'}})
        self.assertEqual({'imports': [], 'inputs': ['rna'], 'comments': {'header': 'kurt'}}, obs)

    def test_parse_gapl_header(self):
        obs = _parse_gapl_header(['import rna\n', 'input rna\n', '\n', 'type Rope = extern\n', '\n'])
        self.assertEqual([], obs['imports'])  # skip rna as this belongs to the rtlib
        self.assertEqual(['rna'], obs['inputs'])

        obs = _parse_gapl_header(['import "ext_alignment.hh"\n', '\n', 'input <raw, raw>\n', 'type Rope = extern\n', 'type typ_ali = (Rope first, Rope second)\n', '\n'])
        self.assertEqual(['ext_alignment.hh'], obs['imports'])
        self.assertEqual(['raw,', 'raw'], obs['inputs'])

        obs = _parse_gapl_header(['/*\n', "# Calif El Mamun's Caravan\n", '<img src="http://www.tabsir.net/images/Kadhimiya.gif" style="di...', '...ity over `+`.\n', '\n', '*/\n', '\n', 'type Rope = extern\n', '\n'])
        self.assertEqual(['raw'], obs['inputs'])
        self.assertEqual(7, len(obs['header']['comments']))

    def test_parse_gapl_signature(self):
        obs = _parse_gapl_signature(['signature sig_rna(alphabet, answer) {\n', '  answer nil(void);\n', '  answer unpaired(Subsequence, answer);\n', '  answer split(answer, answer);\n', '  answer pair(Subsequence, answer, Subsequence);\n', '  choice [answer] h([answer]);\n', '}\n', '\n'])
    ...
    def test_parse_gapl(self):
        ...
        self.assertTrue('cmt: alg enum' in obs['algebras']['alg_enum']['comments'][0])
        self.assertEqual(7, obs['algebras']['alg_editops']['position'])
        self.assertTrue('cmt: alg editops' in obs['algebras']['alg_editops']['comments'][-2])
        ...
        self.assertTrue('cmt: gra needlemanwunsch' in obs['grammars']['gra_needlemanwunsch']['comments'][-2])
        self.assertEqual(5, obs['grammars']['gra_gotoh']['position'])
        self.assertTrue('cmt: gra gotoh' in obs['grammars']['gra_gotoh']['comments'][-2])
        ...
        self.assertEqual(True, all(['example_inputs' not in x for x in obs['instances']['ins_needlemanwunsch_count']['comments']]))
